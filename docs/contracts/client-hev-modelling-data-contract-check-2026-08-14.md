...
Status: Historical implementation check. Its lag and modelling-predictor
conclusions are superseded by `DEC-38` and `DEC-40`; use the 24-25 August
client-confirmed standards for new work.
...
The repository records HEV source-mode requirements in `DEC-37`, `DC-12`, and
`RTM-25`. The later `DEC-38` and `DEC-40` supersede this check's original lag
and model-predictor conclusions.
...
| Lag values | The confirmed set is `0`, `1`, `3`, `6`, and `12`. | This historical implementation exposes only 0 and 1 and therefore no longer satisfies `DEC-38`. | Superseded / Partial | Update UI, server validation, joined schema, provenance, and tests to all five lags. |
...
| Qz values for modelling | Both paths may use Q10z/Q95z; single-site may additionally use raw Q95, while multi-site rejects every raw Flow predictor. | `DEC-40` explicitly supersedes the conflicting predictor distinction in `DEC-21`. | Partial | Align UI and server validation with `DEC-40`; do not expose raw Q10 as a model predictor. |
...
- Enforcing the five valid lag values (`0`, `1`, `3`, `6`, and `12`) at UI/server validation boundaries; the historical 0/1 implementation is incomplete.
- Ensuring joined/analysis data exposes Q10/Q95 raw and Qz fields with provenance for all five lags.
