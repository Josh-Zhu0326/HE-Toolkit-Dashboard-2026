...
**Historical/intermediate browser baseline:** `b0f1bd5` (`docs: align RAW matrix with final recovery status`)
**Historical/intermediate automated baseline:** `19daf7a` (`refactor: consolidate RAW message and test helpers`)
**Historical baseline relationship:** `b0f1bd5` is a documentation-only descendant of `19daf7a`; no production or test behaviour changed between that intermediate automated gate and the browser/manual smoke session.
**Final verified code baseline:** `99b2a04` (committed local verification baseline)
**Corrective review verification:** 2026-08-09 against committed baseline `99b2a04`; implementation and automated results below include the PR #89 review corrections. The full browser suite was not repeated, but the production-rendering change received the focused RAW-18 smoke recorded below.
...
The final automated regression suite was executed against committed baseline `99b2a04`. The working tree was clean except for the temporary `Rplots.pdf` generated by testthat; that artifact was removed and `git status` returned clean. Only the required focused RAW-18 browser smoke was rerun after the production-rendering correction.
...
- Pass with Warning: **2**
- Fail: **0**
- **18/18** established standalone scripts exited with code `0`.
...
```text
docs/testing/evidence/browser-2026-08-09/
```
...
- 165 testthat cases completed; 1,176 / 1,176 expectations passed.
- 0 failures, 0 errors, 0 warnings and 0 skips.
- 18 / 18 established standalone scripts exited `0`: 16 Pass, 2 Pass with Warning and 0 Fail.
- The two historical warning-only script categories remain unchanged; no new warning category was introduced.
...
```text
docs/testing/reports/browser-2026-08-09/RAW-01-25_Final_Browser_Manual_Verification_Report.md
```
