# Dissertation Final Engineering Evidence

This document defines the reproducible evidence run for the dissertation. The
evidence tooling records and tests existing application behaviour; it does not
change the Shiny application's runtime paths, state transitions, or user
interface.

## Evidence question

The final evidence set must allow an independent reviewer to establish:

1. which exact commit was tested;
2. which operating system, R runtime, packages, and commands were used;
3. which automated and boundary behaviours were tested; and
...
This branch adds the evidence tooling and tests only. The repository owner is
...
development verification, not dissertation results.
...
```bash
cd /tmp
git clone <repository-url> hetoolkit-final-test
cd hetoolkit-final-test
...
Rscript --vanilla scripts/run_final_evidence.R \
  <FINAL_SHA> \
  /Users/katherinal/Desktop/SummerProject/HE-Toolkit-Dashboard-2026-evidence/<FINAL_SHA>/<ATTEMPT_ID>
```

The checkout may be detached, but all of these conditions must hold:

- `FINAL_SHA` is a complete 40-character hexadecimal SHA;
- `HEAD`, `origin/main`, and `FINAL_SHA` are identical;
- the worktree is clean before and after the run;
...
One command runs and records:

- dependency installation in the isolated temporary libraries;
- the dashboard startup preflight;
- the complete `tests/testthat` suite;
- every top-level `tests/test_*.R` standalone script in its own R process;
- B01-B07 boundary scenarios; and
- a post-run Git cleanliness check.

The testthat report records test files, cases, expectations, passes, failures,
errors, warnings, skips, and elapsed time. Warnings and skips are not silently
treated as an unqualified pass.
...
| B06 | Failure recovery | A failed model attempt remains explicit and non-current; a reviewed retry creates a new current revision. |
...
`SHA256SUMS` makes later accidental changes detectable. Do not edit a generated
attempt in place. If a command or observation must be corrected, use a new
attempt identifier and retain the earlier attempt as an audit record.
...
unqualified result. Explain each warning or skip in the dissertation evidence
notes and state whether it affects a reported claim.
...
The UI record must use the same final SHA and attempt as the automated results.
If a UI observation fails, retain it as a failed attempt and investigate before
reporting the claim.

## Dissertation reporting rule

Section 3.2 and Table 3.2 must be populated from `manifest.json`,
`testthat-summary.json`, and `boundary-scenarios.csv`, not from historical test
counts. Report the complete commit SHA, execution date, R version, operating
system, cases, expectations, failures, errors, warnings, skips, standalone
script result, and B01-B07 result.

Suggested wording:

> All reported engineering results were reproduced against commit `<FINAL_SHA>`
> in the environment recorded in the accompanying evidence manifest.

The primary evidence run covers the recorded macOS/R environment. It does not,
by itself, claim Windows-launcher validation or successful integration with
authorised external services; those require separately identified evidence.
