...
The customer source metadata contains both `rhs_site_id` and `rhs_survey_id`. However, the confirmed Dashboard metadata contract treats `rhs_survey_id` as the canonical RHS identifier. `rhs_site_id` is retained here only as a source-data field considered during validation testing and is not treated as a supported Dashboard identifier.
...
| FT-01D | Both RHS Identifiers Present | Import metadata containing both `rhs_site_id` and `rhs_survey_id`, whether the values are identical or different | The Dashboard rejects metadata containing both RHS identifier columns and clearly instructs the user to remove `rhs_site_id` and retain `rhs_survey_id` | The current Dashboard treats the two fields as a conflict only when their values differ. This allows non-canonical metadata when the values are identical and does not consistently enforce `rhs_survey_id` as the sole supported RHS identifier. | Fail | BUG-002 |
...
| Test ID | Test Area | Test Step | Expected Result | Actual Result | Status | Evidence |
|---|---|---|---|---|---|---|
| FT-03A | Customer Flow Import Using HDE | Import customer Flow data using `HDE` as the customer-confirmed preferred Flow source | Flow data are retrieved successfully and displayed for the mapped customer sites | HDE Flow import completed successfully and the imported Flow data were displayed with completeness statistics. The R Console reported that 2 duplicate site-input combinations were identified and dropped. The warning did not block the import. | Pass with Warning | OBS-002; `ST-03C_flow_import_warning.png` |
...
| Test ID | Test Area | Test Step | Expected Result | Actual Result | Status | Evidence |
|---|---|---|---|---|---|---|
| FT-04A | Customer WQ Data Import | Import WQ data using all 49 customer `wq_site_id` values | WQ data are retrieved successfully for the mapped customer sites | WQ import completed successfully for the 49-site customer dataset and retrieved 47,290 unique records. The operation took approximately 20 minutes. The customer subsequently confirmed that this duration is reasonable for a 49-site modelling-study workload under the current WQ Data Explorer behaviour. | Pass | Customer-confirmed large-batch performance baseline |

### 8.5 FT-05: RHS Data Import

| Test ID | Test Area | Test Step | Expected Result | Actual Result | Status | Evidence |
|---|---|---|---|---|---|---|
| FT-05A | Customer RHS Data Import Using Temporary Identifier Workaround | Import RHS data using metadata where `rhs_survey_id` values were duplicated into `rhs_site_id` to satisfy the current validator | The RHS workflow should operate using canonical `rhs_survey_id` metadata without requiring or generating `rhs_site_id` | The temporary workaround passed validation and displayed 49 RHS entries. However, the workaround duplicated `rhs_survey_id` values into the unsupported `rhs_site_id` field and therefore does not represent valid behaviour under the confirmed RHS identifier contract. | Fail | BUG-002; test input used a non-canonical workaround |
...
| Test ID | Test Area | Test Step | Expected Result | Actual Result | Status | Evidence |
|---|---|---|---|---|---|---|
| FT-07A | Customer Flow Statistics Calculation | Calculate Flow Statistics using successfully imported customer HDE Flow data | Flow Statistics are calculated and displayed for the imported customer Flow dataset | The calculation failed with `Error in calc_flowstats: Duplicate dates identified`. No Flow Statistics output was generated and the interface remained in a loading state. | Fail | BUG-003; FT-07A_flow_statistics_duplicate_dates_error.png |
...
| FT-07A-R2 | Flow Statistics Calculation with 20-Site Subset | Calculate Flow Statistics using the fixed first 20 customer sites with HDE Flow data | Flow Statistics are calculated and displayed successfully | Flow Statistics calculation completed successfully and the calculated results table was displayed. The progress indicator reached `100%`, and the `Duplicate dates identified` error observed with the 49-site dataset did not occur. | Pass | — |

### 8.8 FT-08: Analysis

| Test ID | Test Area | Test Step | Expected Result | Actual Result | Status | Evidence |
|---|---|---|---|---|---|---|
| FT-08A | Customer Biology–Flow Analysis | Pair processed Biology and Flow data and generate Analysis outputs using the original full customer dataset | Biology and Flow data are paired successfully and Analysis outputs are generated | The original full-dataset test could not complete because Flow Statistics were unavailable after the duplicate-date failure recorded in `BUG-003`. Later fixed 10-site and 20-site subset retests successfully produced Flow Statistics and allowed Analysis pairing to proceed. | Blocked | Blocked for the original 49-site dataset by BUG-003; see FT-08A-R1 and FT-08A-R2 |
| FT-08A-R1 | Biology–Flow Pairing with 10-Site Subset | Pair Biology and Flow data using the 10-site customer subset | Pairing either completes successfully or clearly reports any valid data prerequisite issue | Paired Biology–Flow data were generated. The previous `missing value where TRUE/FALSE needed` error did not occur. The Dashboard displayed a clear date-range warning because some Biology samples preceded the earliest available Flow period. | Pass with Warning | Data time-range mismatch; BUG-001 not reproduced |
...
| Test ID | Test Area | Test Step | Expected Result | Actual Result | Status | Evidence |
|---|---|---|---|---|---|---|
| FT-09A | Customer HEV Plot Generation | Generate HEV plots using valid paired Biology–Flow data | HEV plots are generated successfully | Not re-executed after the 10-site and 20-site Analysis pairing retests. Paired Biology–Flow data became available in the subset retests, so the original blocker is no longer valid for those scenarios. | Not Run | HEV retest required using a successful paired subset |
...
|---|---|---|---|---|---|---|
| FT-01E | Retest of FT-01C | Upload metadata containing `rhs_survey_id` but no `rhs_site_id` | Metadata validation passes and `rhs_survey_id` is accepted as the supported RHS identifier | Not yet executed because the updated RHS validation behaviour has not been implemented | Not Run | Awaiting implementation |
| FT-01F | Retest of FT-01B | Upload metadata containing `rhs_site_id` but no `rhs_survey_id` | Metadata validation does not silently treat `rhs_site_id` as equivalent to `rhs_survey_id` and provides a clear validation message | Not yet executed because the updated RHS validation behaviour has not been implemented | Not Run | Awaiting implementation |
...
| FT-01L | Both RHS Identifier Columns Present with Matching Values | Upload metadata containing both `rhs_site_id` and `rhs_survey_id` with identical values | Metadata validation rejects the file and clearly instructs the user to remove `rhs_site_id` and retain `rhs_survey_id` | Not yet executed because the updated RHS validation behaviour has not been implemented | Not Run | Awaiting implementation |
| FT-01M | Both RHS Identifier Columns Present with Different Values | Upload metadata containing both `rhs_site_id` and `rhs_survey_id` with different values | Metadata validation rejects the file and clearly instructs the user to remove `rhs_site_id` and retain `rhs_survey_id` | Not yet executed because the updated RHS validation behaviour has not been implemented | Not Run | Awaiting implementation |
| FT-01N | External RHS `Survey.ID` Canonicalisation | Import RHS source data containing external `Survey.ID` values and inspect the normalised internal/output schema | External `Survey.ID` is explicitly renamed or mapped to `rhs_survey_id`; the normalised internal/output data contain `rhs_survey_id` only and do not retain or generate `rhs_site_id` | Not yet executed against the updated RHS normalisation workflow | Not Run | Awaiting implementation |