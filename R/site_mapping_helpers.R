...
safe_external_import <- function(operation, required_columns = character()) {
  ...
  if (!is.data.frame(result)) {
    return(list(
      ...
      diagnostic = "The external result was not tabular."
    ))
  }
  if (nrow(result) == 0L) {
    return(list(
  ...
  if (length(missing_columns) > 0L) {
    return(list(
      ...
      data = NULL,
      failure = "invalid_result",
      diagnostic = paste("Missing required result columns:", paste(missing_columns, collapse = ", "))
    ))
  }
  ...
  if (length(unusable_columns) > 0L) {
    return(list(
      ...
      data = NULL,
      failure = "invalid_result",
      diagnostic = paste("Required result columns contain no usable values:", paste(unusable_columns, collapse = ", "))
    ))
  }
