...
workflow_style_tags <- function() {
  shiny::tags$style(shiny::HTML("
    ...
    .workflow-checkpoint-row { padding:.55rem 0; border-bottom:1px solid #edf1ee; }
...
workflow_required_steps_ui <- function(task, stage_index, registry) {
  ...
  # Add a user-facing label here whenever an artifact is added to the registry.
  labels <- c(
  ...
  shiny::tags$ul(
    ...
    lapply(artifact_ids, function(artifact_id) {
      ...
      shiny::tags$li(
        class = "workflow-step",
        shiny::div(
          shiny::strong(labels[[artifact_id]]),
...
workflow_workspace_ui <- function(task_id, current_stage, registry) {
  ...
  shiny::div(
    ...
    shiny::div(
      ...
      shiny::tags$aside(
        class = "workflow-panel",
        shiny::h3("Checkpoint"),
        shiny::div(class = "workflow-checkpoint-row", shiny::strong("Stage status"), shiny::div(stage_status)),
        shiny::div(class = "workflow-checkpoint-row", shiny::strong("Data source"), shiny::div("Shown after validated input or processing")),
        shiny::div(class = "workflow-checkpoint-row", shiny::strong("Data history"), shiny::div("Shown after a real output is generated")),
        shiny::div(class = "workflow-checkpoint-row", shiny::strong("Next action"), shiny::div(workflow_primary_action_label(task_id, current_stage)))
      )
    ),
    shiny::span(
...
workflow_shell_ui <- function(task_id = NULL, current_stage = 1L, registry = new_he_artifact_registry()) {
  # Keep this selector as the only entry point when no Task is active.
  if (is.null(task_id)) {
    return(workflow_task_selector_ui(registry = registry))
  }
  workflow_workspace_ui(task_id, current_stage, registry)
}
