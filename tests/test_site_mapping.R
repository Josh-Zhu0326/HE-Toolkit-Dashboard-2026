...
stopifnot(!"rhs_site_id" %in% names(parsed$data))
...
stopifnot(grepl("Replace it with rhs_survey_id", legacy$error))

matching_dual <- parse_site_metadata("biol_site_id,rhs_survey_id,rhs_site_id\n291,6145,6145")
stopifnot(!is.null(matching_dual$error))
stopifnot(grepl("Remove rhs_site_id", matching_dual$error))

conflicting_dual <- parse_site_metadata("biol_site_id,rhs_survey_id,rhs_site_id\n291,6145,9999")
stopifnot(!is.null(conflicting_dual$error))
stopifnot(grepl("Remove rhs_site_id", conflicting_dual$error))

stopifnot(grepl(
  "Replace it with rhs_survey_id",
  validate_dashboard_site_metadata(data.frame(rhs_site_id = "6145"))
))
stopifnot(grepl(
  "Remove rhs_site_id",
  validate_dashboard_site_metadata(data.frame(rhs_survey_id = "6145", rhs_site_id = "6145"))
))
...
external_rhs <- data.frame(Survey.ID = "R1", HQA = 50, stringsAsFactors = FALSE)
external_local_error <- tryCatch(
  normalise_rhs_records(external_rhs),
  error = function(e) conditionMessage(e)
)
stopifnot(grepl("external RHS interface", external_local_error))
rhs <- normalise_rhs_records(external_rhs, allow_external_survey_id = TRUE)
stopifnot(!"Survey.ID" %in% names(rhs))
stopifnot(!"rhs_site_id" %in% names(rhs))
...
stopifnot(!"Survey.ID" %in% names(temp_rhs))
stopifnot(!"rhs_site_id" %in% names(temp_rhs))
